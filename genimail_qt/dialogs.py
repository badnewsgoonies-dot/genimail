...
class CompanyManagerDialog(QDialog):
    ...
    def _pick_selected_color(self):
        ...
        if not domain:
            QMessageBox.information(self, "No Selection", "Select a domain first.")
            return
        colors = self.config.get("company_colors", {}) or {}
        initial = QColor(colors.get(domain) or "#1f6feb")
        selected = QColorDialog.getColor(initial, self, f"Color for @{domain}")
        if not selected.isValid():
            return
        colors[domain] = selected.name()
        self.config.set("company_colors", colors)
        self.changed = True
        self._load_data()
